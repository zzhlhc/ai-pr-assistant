...
public class ReviewService {
    ...
    public ReviewService(GiteeClient giteeClient, AgentReviewer agentReviewer) {
        this.giteeClient = giteeClient;
        this.agentReviewer = agentReviewer;
    }

    public ReviewResult review(String repo, String commitSha) {
        return review(repo, commitSha, ReviewProgress.none());
    }

    /**
     * @param progress 进度回调。一次评审要跑一分钟以上，把"当前在干什么"暴露出去，
     *                 前端的进度提示才不会从头到尾卡在同一句话上；每一步还会被单独上报，
     *                 页面靠它把完整轨迹实时累积起来。
     */
    public ReviewResult review(String repo, String commitSha, ReviewProgress progress) {
        ...
        return new ReviewResult(verify(outcome.report(), diffs), outcome.usage(), elapsedMillis, outcome.trace());
    }
